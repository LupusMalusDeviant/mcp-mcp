...
- *2026-07-18 (NACHTRAG 3 — Soll-Anforderungen):* Ein zweiter unabhängiger Durchgang hat die zehn Fixes verifiziert (alle real verdrahtet, keine tote DI-Registrierung, kein wirkungsloses Bedienelement) und zusätzlich die **Soll**-Anforderungen geprüft. Nachgezogen: **FR-25** — die Retention stand fest auf 30 Tagen (`new PersistenceOptions()` ohne Binding), ist jetzt über `MCPMCP_AUDIT_RETENTION_DAYS` konfigurierbar; ungültige Werte fallen auf den Default zurück. **NFR-07** — es lief der Default-Textformatter; außerhalb von `Development` schreibt der Host jetzt JSON auf stdout (lokal bleibt Text, weil dort Lesbarkeit zählt). **FR-38** — das Token-Cockpit zeigte nur Token-Last, jetzt zusätzlich den Call-Volumen-Trend über 14 Tage (eine Count-Abfrage je Tag statt Full-Scan; Balken aus CSS statt Diagramm-Bibliothek, eine Abhängigkeit weniger). Dazu die Nacharbeit, dass der Debug-Payload zwar gespeichert, aber in der Logs-UI und im CSV nicht sichtbar war. **FR-02 (SSE-Legacy-Transport) — geklärt, siehe [ADR-0009](../adr/0009-sse-legacy-transport.md):** Die Recherche hat die Frage anders beantwortet als erwartet. Upstream war SSE **bereits erfüllt**, nur unbemerkt: `HttpClientTransportOptions.TransportMode` steht im SDK per Default auf `AutoDetect` und fällt selbsttätig auf HTTP+SSE zurück — der Konnektor hat den Wert nie gesetzt. Das erste Audit hat daraus „kein SSE-Konnektor vorhanden" gelesen, was formal stimmt, aber die falsche Schlussfolgerung war. Jetzt wird `AutoDetect` **explizit** gesetzt, damit ein SDK-Upgrade die Fähigkeit nicht stillschweigend entfernt, per Server über `AllowLegacySse` abschaltbar, mit UI-Schalter. Downstream bleibt es bei Streamable HTTP: `EnableLegacySse` ist im SDK `[Obsolete]`, SSE fehlt HTTP-Backpressure, und SEP-2596 gibt HTTP+SSE die kürzeste Entfernungsfrist aller deprecateten Features (drei Monate nach Final). **Lehre:** „Klasse X existiert nicht" ist noch kein Befund — erst der Blick auf das tatsächliche Laufzeitverhalten sagt, ob eine Anforderung erfüllt ist. Fähigkeiten, die an einem Framework-Default hängen, gehören per Test festgenagelt (`LegacySseSupportTests`).
- *2026-07-18 (NACHTRAG 4 — Abnahme-Audit vor dem Release):* Drei unabhängige Durchgänge (Muss-FRs, Soll/Kann/NFRs, adversariale Testqualität) vor einem korrigierten Release. Ergebnis: keine Hard-Blocker, aber zehn Punkte — auffällig viele davon **Nachweislücken statt Funktionslücken**. Umgesetzt: **FR-08** — `Degraded` war ein Enum-Wert, den nie jemand schrieb; ein Ping-Fehler sprang direkt auf `Failed` und riss die Verbindung samt In-Flight-Calls mit. Jetzt toleriert der Supervisor einen Aussetzer als `Degraded` (Verbindung bleibt geroutet, Zustand sichtbar, auditiert) und eskaliert erst danach. **FR-34** — „Bearbeiten" fehlte in der UI komplett, `ReconfigureAsync` war nur per REST erreichbar (verletzte Z-6). Neu mit Secret-Regel: bestehende Zugangsdaten werden nicht angezeigt, leer gelassene Felder bedeuten „unverändert" (`UpstreamConfigMerge`, im Kern statt in der Razor-Seite, damit testbar). **FR-31** — das Gültigkeitsfenster war in der UI hart `null`. **FR-05** — die Loop-Erkennung versprach „direkt oder transitiv", kann aber nur den direkten Selbstbezug: der Instanz-Header wird beim Verbindungsaufbau *einmal* gesetzt und kennt den auslösenden Request nicht. Behauptung korrigiert, Grenze samt Mitigation (Call-Timeout deckelt den Schaden) im Threat-Model. **NFR-01/02** — der einzige Test, der die Lastschwellen assertiert, war über eine nie gesetzte Env-Variable permanent übersprungen: die NFRs waren faktisch ungeprüft. Jetzt eigener wöchentlicher/manueller CI-Job. **NFR-05** — arm64 wurde behauptet, aber nie gebaut (jetzt `buildx`-Schritt); `docker-compose.yml` hatte ein `depends_on` auf einen nur im Profil aktiven Service, der SQLite-Default war damit je nach Compose-Version defekt (jetzt Override-Datei statt Profile, plus `config`-Prüfung in CI). **NFR-08** — Coverage wurde nur als Artefakt hochgeladen; jetzt Gate auf 80 % Zeilen für `McpMcp.Core` (Ist: 84 %). **NFR-09** — die Protokollversion hing still am SDK; jetzt per Handshake-Test auf „2025-06-18 oder neuer" festgenagelt. **Testhärtung:** Der Postgres-Fang machte *jeden* Fehler zum grünen Skip — auf Linux-CI erzwingt `MCPMCP_REQUIRE_POSTGRES` jetzt das Durchreichen, dazu ein Skip-Zähler als Backstop. Zeitabhängige Negativ-Assertions ersetzt (`StaysFalseAsync`). Neuer Reflection-Test über die Policies aller routbaren Seiten — bei Blazor Interactive Server ist das Seitenattribut die Durchsetzung, darunter prüft kein Service UI-Rollen. **Doku-Fallen entfernt:** Kommentare, die längst Umgesetztes als „folgt" auswiesen — und, sicherheitsrelevant, `IAssetStore.ListAsync` versprach eine RBAC-Filterung, die es nie gab (der `identity`-Parameter wird ignoriert); jetzt steht die Grenze samt Warnung im Vertrag. **Lehre 3:** Grün heißt nicht geprüft. Ein Test, der nie läuft, ist teurer als keiner — er erzeugt Sicherheit, die es nicht gibt.
- *2026-07-18 (NACHTRAG 5 — Kann-Anforderungen entschieden):* Die fünf Kann-Punkte hatte ich in Berichten als „v2" geführt — meine Einordnung, nicht die des Auftraggebers. Nach Vorlage von Aufwand und Nutzen je Punkt entschieden: **FR-41 umgesetzt** (Client-Config-Snippets beim Ausstellen eines Keys, also genau dann, wenn der Key einmalig sichtbar ist; Namen werden auf ASCII normalisiert, weil sie als Schlüssel in fremde Config-Dateien wandern). **FR-16 umgesetzt** (`MCPMCP_MAX_RESULT_CHARS`, Default aus). Die zwei Entscheidungen, die es brauchbar machen, stecken nicht in der Kürzung selbst: Das Ergebnis bleibt **gültiges JSON** — ein roher Abschnitt wäre für den Agenten wertlos, er hätte dann gar nichts statt zu wenig — und die Kürzung ist per `_mcpmcp_truncated` **erkennbar**, denn verschwiegen wäre sie schlimmer als die Größe. Das Audit hält weiter die ungekürzte Größe fest. **FR-04 als Abweichung geschlossen ([ADR-0010](../adr/0010-sampling-elicitation-nicht-durchreichen.md)):** Die Machbarkeitsprüfung ergab, dass beide SDK-Enden existieren, aber die Mitte strukturell fehlt — das Protokoll trägt keine Korrelation, der SDK-Handler bekommt keinen Kontext, und die Upstream-Verbindung ist geteilt, während die Identität schon im Invoker endet. Tragfähig wäre nur ein Pool je (Server, Identität), was die 1:1-Annahme des Supervisors kippt (16–26 PT, ±40 %). Ausschlaggebend war aber nicht der Aufwand: Sampling lässt einen Upstream LLM-Inferenz auf Kosten des Agenten auslösen, und die Spezifikationszusage „ein Mensch kann immer ablehnen" kann ein Gateway zwischen headless Agent und Server nicht einlösen. Ein Feature auszuliefern, dessen Sicherheitsversprechen die Architektur nicht trägt, wäre schlechter als es wegzulassen. **FR-20 und FR-32 zurückgestellt** — bei FR-32 ist nicht die Queue das Problem, sondern der blockierende Call, der dem Timeout aus FR-09 widerspricht; das braucht ein Design, keine Implementierung. - *2026-07-20 (NACHTRAG 6 — Abhängigkeiten auf aktuellen Stand):* **.NET 10 ist LTS** (Support bis 2028-11-14); .NET 11 gibt es nur als Preview mit STS-Zyklus und ist kein Ziel. Neu `global.json` mit **Untergrenze** 10.0.200 + `rollForward: latestFeature` — ein Pin auf eine exakte Version (erst versucht: 10.0.302) bricht jede Maschine, auf der sie nicht installiert ist, und tat das hier sofort. **GitHub Actions** auf die Node-24-Majors gehoben (checkout@v7, setup-dotnet@v6, upload-artifact@v7, setup-buildx-action@v4); `upload-artifact@v5` hätte die Node-20-Warnung *nicht* gelöst, dort war Node 24 nur vorläufig. **Docker-Basis** explizit auf `10.0-noble` gepinnt: Der Default-Alias `10.0` zeigt heute auf Ubuntu 24.04, wandert aber bei einem künftigen Distro-Wechsel weiter. Chiseled bleibt ausgeschlossen, weil das Laufzeit-Image für `/data` eine Shell braucht. **FluentAssertions → AwesomeAssertions 9.5.0:** FluentAssertions steht ab v8 unter kommerzieller Xceed-Lizenz (NFR-10), auf 7.x zu bleiben wäre eine tote Linie gewesen; AwesomeAssertions ist der Apache-2.0-Fork von vor dem Lizenzwechsel und war tatsächlich drop-in — nur Paketname und Namespace, keine einzige Assertion geändert. **xUnit 2.9.3 → xunit.v3 3.2.2:** Testprojekte sind jetzt eigenständige Executables (`OutputType=Exe`), `Xunit.SkippableFact` entfällt zugunsten des eingebauten `Assert.SkipWhen`/`SkipUnless`, `IAsyncLifetime` liefert `ValueTask` und erbt von `IAsyncDisposable`. **Der Umstieg hat einen echten Defekt gefunden:** `State_changes_are_written_to_the_audit_log` bestand isoliert, fiel aber im Gesamtlauf — der Test wartete auf den Zustand und las dann das Audit, während `SetState` den Zustand *vor* dem Eintrag sichtbar macht. Ein selbstgebautes Rennen, das nur auf einer unbelasteten Maschine gewann; jetzt wird auf das Ereignis gewartet. Danach dreimal hintereinander grün. Bewusst **nicht** übernommen: `xUnit1051` verlangt an jedem Aufruf mit optionalem Token den TestContext-Token — die 198 Stellen mit ohnehin übergebenem Token sind umgestellt, die ~360 impliziten wären Lärm ohne Nutzen (die Wartehelfer haben eigene Timeouts). Nachgeprüft statt angenommen: Die TRX-Datei meldet auch unter v3 `outcome="NotExecuted"`, das CI-Gate auf übersprungene Tests greift unverändert.

- *2026-07-20 (WP9 — Secret-Guardrail, [ADR-0011](../adr/0011-secret-erkennung-als-guardrail.md)):* Neue Stufe im `ToolInvoker` — dem einzigen Weg zu einem Call, womit MCP, REST und UI in einem Zug abgedeckt sind. MCP-MCP bleibt der Kern; die Prüfung ist ein Filter darin, kein zweites Produkt. Verträge in Abstractions (`IContentGuard`, `GuardRule`, `GuardFinding`, `IGuardRuleStore`, `GuardOptions`), Erkennung im Core, Regeln in der DB mit Write-Through wie bei den Redaction-Mustern — also **hot-swappable ohne Neustart**. Pipeline: `SearchValues`-Multiscan (Ordinal) → Keyword je Regel → Regex mit `NonBacktracking` + Timeout. Gemessen ~0,05 ms, also unter 1 % des 7-ms-Budgets. Neuer Status **`GuardBlocked`**, bewusst nicht `Denied`: In der Ergebnis-Richtung ist der Upstream-Call bereits gelaufen, der Seiteneffekt also eingetreten — die Fehlermeldung sagt „ausgeführt, Ergebnis zurückgehalten, NICHT wiederholen", und das Audit hält den stattgefundenen Call fest. Sonst wiederholt ein Agent den Aufruf und legt dasselbe Issue zweimal an. Startregelsatz aus gitleaks (MIT; trufflehog scheidet wegen AGPL-3.0 aus, NFR-10), kuratiert statt vollständig — jede kompilierte Regel kostet ~530 KB. UI mit geführtem Editor als Standard, Freitext-Regex hinter `MCPMCP_GUARD_ALLOW_CUSTOM_PATTERNS`, Validierung beim Speichern, Modus pro Regel, globaler Not-Aus. **Lehre 5:** Die schwierigste Frage war nicht die Erkennung, sondern was „blockieren" in der Ergebnis-Richtung überhaupt bedeutet — und dass eine Fehlermeldung, die den bereits ausgeführten Call verschweigt, einen doppelten Seiteneffekt provoziert.

**Lehre 4:** Scope-Entscheidungen sind keine technischen Urteile. „Kann" heißt nicht „ich entscheide das still" — jeder offene Punkt bekommt einen dokumentierten Status, sonst ist später nicht unterscheidbar, was entschieden und was vergessen wurde.
